...
class VideoProcessor:
    ...
    if self.colors_dict is None:
        ...
        color = self.colors_dict.get(class_id)
        return color
    
    #Draw Arc Under Player
    def draw_player_arc(self, frame, class_id, radius, start_angle, end_angle, thickness=2, angle=0, x1, x2, y1, y2):
    width = bbox_width(x1, x2)
    center = bbox_center_bottom(x1, y1, x2, y2)
    color = class_color(class_id, self.colors_dict)
    ...
    def process_frame(self, frame):
        ...
        for detection, detection_class in zip(detections, detections_class):
            ...
            color = class_color[int(detection_class)]